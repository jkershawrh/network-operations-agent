...
export function SceneRenderer({ scene, brand }: { scene: SceneConfig; brand: { primary: { logo: string; alt: string }; partner: { logo: string; alt: string } } }) {
  ...
  if (scene.type === 'evidence-payoff') {
    ...
    return <SceneFrame scene={scene}><div className="evidence-payoff">
      {evidence.length ? <><div className="evidence-payoff-status"><span className="source-badge source-live">LIVE</span><strong>{evidence.length} infrastructure investigations completed</strong></div><div className="proof-equation"><div><span>ONE ALARM</span><strong>PTP degraded</strong></div><b>+</b><div><span>{evidence.length} LIVE CONDITIONS</span><strong>{evidence.reduce((sum, item) => sum + item.observationCount, 0)} observations</strong></div><b>=</b><div className="proof-equation-result"><span>TWO SUPPORTED DECISIONS</span><strong>{causes} causes</strong></div><div className="proof-safety"><span>ZERO AUTOMATED ACTIONS</span><strong>{safe ? 'Human authority preserved' : 'Review required'}</strong></div></div><div className="journey-evidence">{evidence.map((item) => <div key={item.scenarioId}><span>{item.scenarioId}</span><strong>{item.cause}</strong><small>{item.observationCount} observations · {item.latencyMs}ms · action executed: {String(item.actionExecuted)}</small></div>)}</div></> : <div className="evidence-empty"><span className="source-badge source-offline">NOT RUN</span><strong>{scene.emptyState}</strong></div>}
