...
This repository contains two deliberately separate delivery paths. The persistent presenter environment runs the 5–7 minute story and bounded live guided proof. The Network Operations hands-on lab is ordered separately in Partner AI Launchpad and receives its own provisioned environment and network. The presentation does not embed the lab, its authoring controls, or its failure-injection path. The lab adds a learner-built upstream-clock scenario, a fourth MCP diagnostic, controlled dependency failures, a reliability qualification report, and a NOC decision brief.

The `presentation/` application tells the quickstart as a Red Hat × Intel interactive demo story. The built presenter runtime serves it at `/story/` beside the bounded live proof, then closes with a link to the separately orderable Launchpad lab. It preserves the same safety and evidence boundaries, calls `POST /api/investigate` for the two approved scenarios, explains the operating mechanisms inline, and builds its payoff from the causes, observation counts, request timing, and action boundary returned by the current Flightpath session. It never hard-codes infrastructure metrics. Run it with `npm run presentation:dev` and validate it with `npm run presentation:check`.
...
### Separate Launchpad lab

The lab source is under `showroom-lab/` and is built with `npm run build:lab`, but it is not enabled in the presenter deployment. Participants order *Intel AI 301: Build an Evidence-Backed Network Operations Agent* from Partner AI Launchpad, which provisions the lab's separate environment and network. The lab adds bounded synthetic authoring and failure-injection APIs; it still cannot execute remediation or accept arbitrary MCP tools. See the [lab contract](contracts/lab-contract.yaml).
