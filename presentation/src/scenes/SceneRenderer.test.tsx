...
describe('SceneRenderer', () => {
  ...
  it('opens and closes the phase-aware technical topology', () => {
    ...
    expect(screen.getByLabelText('Live journey progress').querySelectorAll('button')).toHaveLength(6)
  ...
  it('makes the agent, policy, LLM, and human boundaries explicit at decision time', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Run live investigation/ }))
    await screen.findByText('What did the agent find?')
    fireEvent.click(screen.getByRole('button', { name: /Explain the evidence policy/ }))
    expect(await screen.findByLabelText('MCP evidence qualification')).toHaveTextContent('hardware-1')
    fireEvent.click(screen.getByRole('button', { name: /Apply the policy/ }))
