...
test('live journey runs both conditions and accumulates returned evidence', async ({ page }, testInfo) => {
  ...
  await page.route('**/api/investigate', (route) => {
    ...
    return route.fulfill({ json: {
      ...
      model_draft: {
        ...
        summary: hardware ? 'The hypothesis suggests a hardware timing issue as the cause, primarily supported by evidence of a present NIC timestamp fault. Current observations indicate a timing alarm while the platform timing service remains healthy. Approved history provides context, but the diagnosis remains provisional and requires human review.' : 'The hypothesis suggests a platform timing issue as the cause, primarily supported by evidence of a present host timing-service fault. Current observations indicate a timing alarm while NIC timestamping remains healthy. Approved history provides context, but the diagnosis remains provisional and requires human review.',
  ...
  await page.getByRole('button', { name: /Explain the evidence policy/ }).click()
  await expectActionsAnchored()
  await expect(page.getByText('Evidence policy', { exact: true }).first()).toBeVisible()
  await expect(page.getByLabel('MCP evidence qualification')).toContainText('READ-ONLY MCP')
  await expect(page.getByLabel('MCP evidence qualification')).toContainText('EVIDENCE CONTRACT')
  await expect(page.getByLabel('MCP evidence qualification')).toContainText('DETERMINISTIC RULE')
  await expect(page.getByLabel('MCP evidence qualification')).toContainText('hardware-1 · present')
  await expect(page.getByLabel('MCP evidence qualification')).toContainText('live-test')
  await expect(page.getByLabel('MCP evidence qualification')).toContainText('checked-in application rule')
  await expect(page).toHaveScreenshot('live-journey-evidence-policy.png', { fullPage: true })
  await page.getByRole('button', { name: 'Apply the policy →', exact: true }).click()
  await expectActionsAnchored()
  ...
  await expect(page.getByText(/hardware timing issue as the cause/)).toBeVisible()
  ...
  await expect(page.getByText(/ms end-to-end/)).toBeVisible()
  const defaultViewport = page.viewportSize()!
  await page.setViewportSize({ width: 1135, height: 693 })
  const compactNext = page.getByRole('button', { name: 'Run changed condition →', exact: true })
  await expect(compactNext).toBeVisible()
  ...
  await page.getByRole('button', { name: 'Run changed condition →', exact: true }).click()
  ...
  await expect(page.getByText(/platform timing issue as the cause/)).toBeVisible()
  await expect(page.getByText('PROMPT IN')).toBeVisible()
  await expect(page.getByText('DRAFT OUT')).toBeVisible()
  await expect(page.getByText('11677ms inference')).toBeVisible()
  await expect(page).toHaveScreenshot('live-journey-changed-condition.png', { fullPage: true })
  ...
  await expect(page.getByText('PROMPT IN')).toBeHidden()
  await expect(page.getByText('DRAFT OUT')).toBeHidden()
  ...
  await expect(page.getByText('THE RESULT')).toBeHidden()
  ...
  await expect(page.getByLabel('Live proof metrics')).toContainText('INVESTIGATIONS')
  await expect(page.getByLabel('Live proof metrics')).toContainText('OBSERVATIONS')
  await expect(page.getByLabel('Live proof metrics')).toContainText('AVG ALARM → REVIEW')
  await expect(page.getByLabel('Live proof metrics')).toContainText('AVG MODEL INFERENCE')
  await expect(page.getByLabel('Live proof metrics')).toContainText('AUTOMATED ACTIONS')
  await expect(page).toHaveScreenshot('evidence-payoff-metrics.png', { fullPage: true })
  await expect.poll(() => page.evaluate(() => document.documentElement.scrollHeight <= window.innerHeight)).toBe(true)
})
